...
function matchMediaType(mediaType, accepts) {
  return (
    ...
    accepts.some((accept) => {
      ...
      return (
        (trimmedAccept.includes("*") && mediaType.startsWith(prefixAccept)) ||
...
function checkMaxFileSize(maxFileSize, files) {
  const sizeLimit =
    typeof maxFileSize === "string" ? getSizeInBytes(maxFileSize) : maxFileSize;
...
function checkAccept(acceptedMediaTypes, files) {
  ...
  const invalidFile = Array.from(files).some(
    (file) => !matchMediaType(file.type, acceptedMediaTypes)
  );
  return !invalidFile;
}
...
function fileValidation(input, files) {
  ...
  if (constraint.length) {
    const finalMessage =
      wrapper.dataset[constraint] ||
      errorMessage ||
      defaultErrorMessages[constraint];
    input.setCustomValidity(finalMessage);
    updateOrCreateInvalidMsg(input, finalMessage);
  } else {
...
function createFileHandler(allFiles, input) {
  return {
    ...
    attachFiles: (inputEl, files) => {
      ...
      const newFileElements = newFiles.map((file, index) =>
        fileElement(file, index + currentLength)
      );
      const fileListElement = inputEl
        .closest(".field-wrapper")
        .querySelector(".files-list");
      if (multiple) {
      ...
      if (file.data) {
        ...
        */
        if (lastIndex >= 0 && url.indexOf("?") === -1) {
          ...
          url = `${url.substr(0, lastIndex)}/${encodeURIComponent(
            url.substr(lastIndex + 1)
          )}`;
        }
      }
      window.open(
...
export default async function decorate(fieldDiv, field, htmlForm) {
  ...
  if (field.value) {
    const preFillFiles = Array.isArray(field.value)
      ? field.value
      : [field.value];
    const dataTransfer = new DataTransfer();
    const file = new File([preFillFiles[0].data], preFillFiles[0].name, {
      ...preFillFiles[0],
    });
